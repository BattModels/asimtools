...
def enthalpy_vs_pressure(
    image: Dict,
    ev_csv: str,
    plogspace: Tuple[float,float,float] = (-6,-1,100),
    eos_name: str = 'vinet',
) -> Tuple[None,Dict]:
    ...
    # Plot p(V)
    _, ax = plt.subplots()
    ax.plot(p_samples / GPa, v_samples, 'o-')
    ...
    # Plot H(V)
    _, ax = plt.subplots()
    ax.plot(hp_data[:,0], hp_data[:,1], 'o--')
    plt.xscale('log')
    ax.set_ylabel('Enthalpy (eV)')
    ax.set_xlabel('Pressure (eV/$\AA^3$)')
    plt.savefig('enthalpy_vs_pressure.png')
    results = {'files': {
        'enthalpy_vs_pressure': 'enthalpy_vs_pressure.csv'
    }}
    return results
